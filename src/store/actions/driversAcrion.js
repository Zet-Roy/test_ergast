...
export const fetchDrivers = () => async dispatch => {
    ...
    try {
        ...
        dispatch({
            type: types.FETCH_DRIVERS_SUCCESS, payload: {
...
export const fetchMoreDrivers = ({ offset }) => async dispatch => {
    try {
        const { data } = await request.get(`f1/drivers.json?limit=10&offset=${offset}`)

        dispatch({
            type: types.FETCH_MORE_DRIVERS_SUCCESS, payload: {
                list: data.MRData.DriverTable.Drivers,
